...
const GearOrganizer = () => {
  ...
  const addGearPack = (pack) => {
    setGearList((prevList) => {
      ...
      const itemsToAdd = newItems.map((item) => ({ name: item, packed: false }));
  ...
  const removeGearItem = (itemName) => {
    setGearList((prevList) => prevList.filter((gear) => gear.name !== itemName));
    setToast({
      show: true,
        ...
        <main className="w-full lg:w-3/4 pr-5">
          ...
          {selectedFeature === 'predefined' && (
            <section className="mb-12">
              <h2 className="text-2xl font-semibold mb-6 text-gray-800">Predefined Gear Packs</h2>
                  ...
                  >
                    <div>
                      <div className="flex items-center mb-4">
                        {pack.icon}
                        <h3 className="text-xl font-bold ml-2 text-gray-800">{pack.name}</h3>
                      </div>
          ...
          {selectedFeature === 'custom' && (
            <section className="mb-12">
              <h2 className="text-2xl font-semibold mb-6 text-gray-800">Your Custom Gear Packs</h2>
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                {/* Custom Gear Packs */}
                {customGearPacks.length === 0 ? (
                  <p className="text-gray-600">No custom gear packs created yet. Click "Create Gear Pack" to get started.</p>
                ) : (
          ...
          {selectedFeature === 'reminders' && (
            <section className="mb-12">
              <h2 className="text-2xl font-semibold mb-6 text-gray-800">Reminders</h2>
              <Reminders />
            </section>
          )}
          ...
          {selectedFeature === 'group' && (
            <section className="mb-12">
              <h2 className="text-2xl font-semibold mb-6 text-gray-800">Group Packing</h2>
              <GroupPacking />
            </section>
          )}
          ...
          {/* Gear List Section */}
          <section>
            <h2 className="text-2xl font-semibold mb-6 text-gray-800">My Gear List</h2>
            {gearList.length === 0 ? (
            ...
            <div className="flex min-h-full items-center justify-center p-4 text-center">
              ...
              >
                <Dialog.Panel className="w-full max-w-md transform overflow-hidden rounded-2xl bg-yellow-100 p-6 text-left align-middle shadow-xl transition-all">
                  ...
                  <div className="mt-4">
                    <GearPackForm onCreate={handleCreateGearPack} onClose={() => setIsModalOpen(false)} />
                  </div>
                </Dialog.Panel>
              </Transition.Child>
            </div>
